...
For a new engineer or hosting organization:

1. clone repository
2. create Artifact Registry repository if not already provisioned
3. build and push service images
4. prepare `terraform.tfvars`
5. run `terraform init`, `plan`, and `apply`
6. open Streamlit UI and validate default-bucket dataset loading
7. upload a sample dataset and verify query/search behavior

## Suggested next engineering tasks

- finalize evaluation framework and metrics pipeline
- move persistent vector/index storage to a cleaner cloud-backed design
- tighten IAM and public exposure for production use
- document exact one-hour deployment path for handoff
- add structured experiment logging and comparison tables

## License / usage

Add the appropriate project license and organizational usage guidance here.
